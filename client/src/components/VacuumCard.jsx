 ...
 * Contrôle des aspirateurs robots avec système de navigation
 */
const VacuumCard = ({
  vacuumData = [],
  onUpdateAppareil,
  ...
  // =========================================================
  // 1. SÉCURITÉ : Vérification si la liste est vide
  // =========================================================
  if (!Array.isArray(vacuumData) || vacuumData.length === 0) {
    return (
  ...
  const safeIndex =
    currentIndex >= 0 && currentIndex < vacuumData.length
      ? currentIndex
      : 0;

  const currentVacuum = vacuumData[safeIndex];

  // Réinitialisation automatique si la liste change
  useEffect(() => {
  ...
  // =========================================================
  // 3. EXTRACTION DES DONNÉES DE L'ASPIRATEUR ACTUEL
  // =========================================================
  const isOn = currentVacuum?.status === 'ENLIGNE';

  const batterie =
    currentVacuum?.chargeBatterie !== undefined
      ? currentVacuum.chargeBatterie
      : 100;

  const isCharging = currentVacuum?.estEnCharge || false;

  // Mode actuel sécurisé
  const currentMode =
    currentVacuum?.modeNettoyage || 'STANDARD';

  // =========================================================
  // DICTIONNAIRE DES DESCRIPTIONS DE MODES
  // =========================================================
  const modeDescriptions = {
    SILENCIEUX:
      "Aspiration discrète pour un nettoyage silencieux, idéal pour la nuit.",

    STANDARD:
      "Puissance équilibrée idéale pour un nettoyage quotidien efficace.",

    TURBO:
      "Puissance maximale pour les tapis et les saletés incrustées (consomme plus)."
  };

  // =========================================================
  // NAVIGATION ENTRE LES ASPIRATEURS
  // =========================================================
  const nextVacuum = () => {
    setCurrentIndex((prev) =>
      prev === vacuumData.length - 1 ? 0 : prev + 1
    );
  };

  const prevVacuum = () => {
    setCurrentIndex((prev) =>
      prev === 0 ? vacuumData.length - 1 : prev - 1
    );
  };

  // =========================================================
  // MISE À JOUR PARTIELLE DE L'APPAREIL
  // =========================================================
  const updateVacuumProperty = (updates) => {
    ...
    const vacuumId =
      currentVacuum._id || currentVacuum.id;

    if (!vacuumId) return;

    onUpdateAppareil(vacuumId, updates);
  };
  ...
  const togglePower = () => {
    ...
    const nextStatus =
      isOn ? 'HORSLIGNE' : 'ENLIGNE';

    updateVacuumProperty({
      status: nextStatus
    });
  };
  ...
  const handleModeChange = (modeName) => {
    ...
    updateVacuumProperty({
      modeNettoyage: modeName.toUpperCase()
    });
  };

  return (
    <div
      className={`relative w-full bg-[#e5e5e5]/60 backdrop-blur-md rounded-[45px] p-6 shadow-xl flex flex-col justify-between h-full transition-all duration-500 select-none ${className}`}
    >
      ...
      <div className="flex justify-between items-center px-2 z-10">
        ...
        <div className="flex flex-col">
          ...
          <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider mt-0.5">
            {vacuumData.length} appareils
            {currentVacuum?.marque
              ? ` • ${currentVacuum.marque}`
              : ''}
          </span>
        </div>
      ...
      <div className="flex items-center justify-between my-4 relative h-32">
        ...
        <button
          onClick={prevVacuum}
          className="p-1.5 hover:bg-black/5 rounded-full transition-colors z-20"
        >
        ...
        <div className="relative flex justify-center items-center w-full">
          ...
          <img
            ...
            className={`w-full max-w-[150px] h-auto object-contain transition-all duration-500 z-10 ${
              isOn
                ? 'opacity-100 drop-shadow-2xl scale-105'
                : 'opacity-40 brightness-95'
            }`}
          />
      ...
      <div className="flex flex-col mb-3">
        ...
        <div
          className={`w-full bg-white/40 rounded-2xl p-1.5 flex gap-1 border border-white/40 transition-opacity duration-300 z-10 ${
        ...
        {isOn && (
          <p className="text-[10px] text-gray-500 italic mt-2 px-2 text-center transition-all duration-300">
            {
              modeDescriptions[currentMode.toUpperCase()]
              || modeDescriptions.STANDARD
            }
          </p>
        )}
