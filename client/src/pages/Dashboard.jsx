...
import axios from 'axios'; // زدنا axios هنا
...
export default function Dashboard({ unreadCount }) {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // كنجيبو البيانات الحقيقية من MongoDB
        ...
        // إيلا فشل السيرفر، كنرجعو لـ localStorage كحل احتياطي
    ...
    // باش إيلا بدلتي السمية فـ Tab أخرى، يتحدث الـ Dashboard فالبلاصة
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ fontSize: 'clamp(11px, 1.5vw, 13px)', color: '#9ca3af', margin: '4px 0 0 0' }}>
            Génerer votre maison intelligente facilement.
          </p>
        </div>
      ...
      {/* باقي الكود (Stats, AC, Light, Lock...) كيبقى هو هو بدون تغيير */}
      <div style={{ display: 'flex', alignItems: 'stretch', gap: '10px', flexWrap: 'wrap' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(110px, 1fr))', gap: '10px', flex: 1, minWidth: 0 }}>
          {[{ label: 'Temp. intérieure', value: '24°C', icon: <Thermometer size={16} color="#9ca3af" /> },
            ...
            { label: 'Energie solaire', value: '78 kwh', icon: <Sun size={16} color="#9ca3af" /> }].map((s, i) => (
      ...
      <div className="main-grid" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: '16px', alignItems: 'start' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: '8px' }}>
              ...
              <div style={{ display: 'flex', background: '#f3f4f6', borderRadius: 10, padding: 3, gap: 2 }}>
                {['Jour', 'Mois', 'Années'].map(t => (
                  <button key={t} onClick={() => setActiveTab(t)} style={{ fontSize: 11, fontWeight: 600, padding: '4px 10px', borderRadius: 7, border: 'none', cursor: 'pointer', background: activeTab === t ? '#687586' : 'transparent', color: activeTab === t ? 'white' : '#1a1a2e' }}>{t}</button>
                ))}
              </div>
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ background: '#EAEAEA', borderRadius: 24, padding: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 1px 4px rgba(0,0,0,0.05)', minHeight: '280px' }}>
            <div style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontSize: 16, fontWeight: 700, color: '#1a1a2e' }}>Lumière ES</span><Toggle on={lightOn} onToggle={() => setLightOn(!lightOn)} /></div>
            ...
            <div style={{ width: '100%', background: '#1a1a2e', borderRadius: 50, height: '45px', display: 'flex', alignItems: 'center', padding: '0 10px', position: 'relative', marginTop: 10 }}>
              ...
              <div style={{ position: 'absolute', left: `${Math.max(8, Math.min(92, lightVal))}%`, transform: 'translateX(-50%)', width: '35px', height: '35px', background: 'white', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 4px rgba(0,0,0,0.2)', zIndex: 2, pointerEvents: 'none' }}><Lightbulb size={18} color={lightOn ? "#fbbf24" : "#cbd5e1"} /></div>
              <span style={{ marginLeft: 'auto', marginRight: 15, color: 'white', fontSize: 13, fontWeight: 600, opacity: 0.5 }}>{lightVal}%</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ background: '#EAEAEA', borderRadius: 24, padding: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 1px 4px rgba(0,0,0,0.05)', minHeight: '320px' }}>
            ...
            <div style={{ width: '100%', background: '#1a1a2e', borderRadius: 50, height: '45px', display: 'flex', alignItems: 'center', padding: '0 10px', position: 'relative', marginTop: 10 }}>
              ...
              <div style={{ position: 'absolute', left: locked ? '10%' : '90%', transform: 'translateX(-50%)', width: '35px', height: '35px', background: 'white', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 4px rgba(0,0,0,0.2)', transition: 'left 0.3s cubic-bezier(0.4, 0, 0.2, 1)', zIndex: 2, pointerEvents: 'none' }}>{locked ? <Lock size={18} color="#1a1a2e" /> : <Unlock size={18} color="#ef4444" />}</div>
          ...
          <div style={{ background: '#EAEAEA', borderRadius: 24, padding: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 1px 4px rgba(0,0,0,0.05)', minHeight: '280px' }}>
            <div style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontSize: 16, fontWeight: 700, color: '#1a1a2e' }}>Aspirateur</span><Toggle on={vacOn} onToggle={() => setVacOn(!vacOn)} /></div>
            ...
            <div style={{ width: '100%', background: '#1a1a2e', borderRadius: 50, padding: '8px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 10 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}><span style={{ fontSize: 14, fontWeight: 700, color: 'white', lineHeight: 1 }}>69%</span><span style={{ fontSize: 10, color: 'rgba(255,255,255,0.5)', fontWeight: 400 }}>Batterie</span></div>
            </div>
          </div>
        </div>
      </div>
